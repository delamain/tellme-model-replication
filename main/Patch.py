...
class Patch(Steppable):
    ...
    def valid_position(self, model, position):
        return model.environments["agent_env"].xsize > position[0] >= 0 and model.environments["agent_env"].ysize > \
               position[1] >= 0
    ...
    def update_SEIR_persons_new_infections_second(self, SEIR_lambda):
        for agent in self.agents:
            if (
                    agent.exposed == False and agent.infected == False and agent.immune == False and agent.vaccinated == False):
                random_float = random.uniform(0, 1.0)
