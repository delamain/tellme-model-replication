...
def color_patches(patches_to_color, max_popn):
...
class DisplayModel(Steppable):
    ...
    def step_prologue(self, model):
        ...
        color_patches(region.live_patches, region.global_population)
    ...
    def write_results_to_csv(self):
        ...
        with open('model_results.csv', mode='w') as model_results:
            ...
            for int in range(0, self.number_of_epochs_count):
                employee_writer.writerow([self.global_number_of_epochs[int], self.global_num_incidence_array[int],
                                          self.global_prevalence_array[int], self.regions_affected_by_epidemic[int]])
